...
class GlobalAttention(nn.Module):
    def __init__(self,
        ...
        self.query = nn.Linear(hidden_size, self.all_head_size) #线性层将输入特征映射到查询向量空间
    ...
    def forward(self, locx, glox):#前向传播函数，locx（局部特征）和glox（全局特征）
        locx_query_mix = self.query(locx) #通射到查询向量空间
        glox_key_mix = self.key(glox)
        glox_value_mix = self.value(glox)

        query_layer = self.transpose_for_scores(locx_query_mix) #重排成注意力分数矩阵的形状
...
class GlobalLocalBrainAge(nn.Module):
    def __init__(self, inplace,
        ...
        self.freqout = nn.Linear(3, 1)
        # self.freqout = nn.Linear(2, 1)
    ...
    def forward(self, xinput):
        ...
        # batch_size, channels, height, width = xinput.size()
        # xfreq = torch.zeros(batch_size, channels, height, width)
        # xfreq = xfreq.to(device)
        # for i in range(batch_size):
        #     y = transforms.ToPILImage()(xinput[i])
        #     y = y.convert("L")
        #     y = np.array(y)
        #     f = np.fft.fft2(y)
        ...
        xfreq = torch.fft.fft2(xinput, dim=(2, 3)).real.to(torch.float32)
        xfreq = torch.fft.fftshift(xfreq)
        xfreq = torch.flatten(self.avg(xfreq), 1)
        # xfreq = xfreq.to(device)
        freq = self.freqout(xfreq)

        # freqinput = torch.sum(xinput, dim=1, keepdim=True) / 3.0
        # input_permuted = freqinput.squeeze(1)
        # # input_complex = input_permuted.type(torch.complex64)
        # freq_representation = torch.fft.fftn(input_permuted, dim=(1, 2))  # 对 height 和 width 维度进行傅里叶变换
        # xfreq = torch.stack((freq_representation.real, freq_representation.imag), dim=-1)
        # xfreq = xfreq.permute(0, 3, 1, 2)
        # xfreq = self.freq_feat(xfreq)
        # xglo = (xglo + xfreq) / 2
        ...
        for y in range(0, H - self.patch_size, self.step): #高度方向
            for x in range(0, W - self.patch_size, self.step): #宽度方向
                locx = xinput[:, :, y:y + self.patch_size, x:x + self.patch_size] #局部区域
                ...
                for n in range(self.nblock):
                    ...
                    tmp = self.attnlist[n](xloct, xglot) #注意力机制融合特征
