...
#     plt.figure(figsize=(10,8))
#     plt.subplot(211)
#     plt.plot(loss_list[9:], color="blue", label="loss")  # 从epoch=10开始绘制
#     plt.plot(val_loss_list[9:], color="orange", label="val_loss")
#     plt.legend()
#     plt.subplot(212)
#     plt.plot(acc_list[9:], color="blue", label="acc")
#     plt.plot(val_acc_list[9:], color="orange", label="val_acc")
#     plt.legend()
#
#     plt.savefig(save_path)
#     plt.close()
...
def accuracy(y_hat, y):
    ...
    # # 将预测值四舍五入到整数
    # rounded_pred = torch.round(y_pred).type(torch.int64)
    # # 将真实值转换为整数
    # y_true_int = y_true.type(torch.int64)
    # # 计算准确预测的比例
    # accuracy = torch.sum(torch.abs(rounded_pred - y_true_int) <= 1).item() / y_true.shape[0]
    # return accuracy
...
class WarmupCosineSchedule(LambdaLR):
    ...
    def lr_lambda(self, step):
        if step < self.warmup_steps: #预热
            return float(step) / float(max(1.0, self.warmup_steps)) #线性增长
        progress = float(step - self.warmup_steps) / float(max(1, self.t_total - self.warmup_steps)) #当前步数相对于总步数减去warmup步数的进度
