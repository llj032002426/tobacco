...
def eval():
    ...
    state_dict = torch.load("./middle/models/hum_ps96_n6_noFFT-best.pth", map_location=device)
    ...
    dataset = BatchDataset("", "", "all2_hum", transform=transform)
    loader = torch.utils.data.DataLoader(dataset, batch_size=16, shuffle=False,num_workers=4, pin_memory=True)

    # 保存结果
    fp = open("./results/hum_ps96_n6_noFFT.csv", "w", encoding="utf-8-sig")
    # fp = open("./results/ps96_hum5_avgall.csv", "w", encoding="utf-8-sig")
    # fp2 = open("./results/ps96_hum5_avg.csv", "w", encoding="utf-8-sig")
    # fp = open("./results/105_avgall.csv", "w", encoding="utf-8-sig")
...
def metrics():
    ...
    # total = 0
    # loss = 0
    # with open("./results/ps96_hum5_avgall.csv", "r", encoding="utf-8-sig") as f:
    ...
    #             count += 1
    # with open("loss_acc_ps96_hum5_avgall.txt", 'a', encoding="utf-8-sig") as f:
    ...
    total2 = 0
    loss2 = 0
    with open("./results/hum_ps96_n6_noFFT.csv", "r", encoding="utf-8-sig") as f:
    # with open("./results/105_avgall.csv", "r", encoding="utf-8-sig") as f:
        ...
        for line in f:
            ...
            if abs(float(time_gt) - float(time_pd)) <= 1:
                count2 += 1
    with open("./loss_acc/hum_ps96_n6_noFFT.txt", 'a', encoding="utf-8-sig") as f:
